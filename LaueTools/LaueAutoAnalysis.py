...
def SimulateResult_withMiller(
    grain, emin, emax, defaultParam, pixelsize=165.0 / 2048, dim=(2048, 2048)):
    ...
    TwicethetaChi = LAUE.filterLaueSpots(Spots2pi,
                                        fileOK=0,
                                        fastcompute=0,
                                        detectordistance=defaultParam[
                                            0
                                        ],  # TODO: check if it is really the parameter set we need
...
def RefineUB(linkedspots_link,
    ...
    _twth, _chi = (
        np.take(data[0], exp_indices),
        np.take(data[1], exp_indices),
    ...
    if results != None:
        ...
        residues_non_weighted = FitO.error_function_on_demand_strain(
            results,
            Data_Q,
            allparameters,
            arr_indexvaryingparameters,
...
def GetStrainOrient(orientmatrix, Bmatrix, elem_label, emax_simul, veryclose_angletol, data, addoutput=0,
    ...
    res_assoc = matchingrate.SpotLinks(twicetheta_exp,
                                        chi_exp,
    ...
    if not res_assoc:
        ...
        linkResidues_link, Energy_Exp_spot, fields, ) = res_assoc
...
def test_indexation_refinement_Ge():
    ...
    (newUBmat, newUmat, newBmat, deviatoricstrain,
    RR, linkResidues_fit, pixdev, nb_fitted_peaks) = GetStrainOrient(orientmatrix,
                                                                        B,
                                                                        elem_label,
                                                                        emax_simul,
                                                                        veryclose_angletol,
...
def RefineCalibParameters(linkedspots,
    ...
    _delta, _newmatrix, SpotsData) = FitO.error_function_on_demand_calibration(
                                                                        results,
                                                                        Data_Q,
                                                                        allparameters,
                                                                        arr_indexvaryingparameters,
...
def test_peaksearch_index_refine():
    ...
    prefix = "autotest_Ge"
    Isorted, fitpeak, localpeak = rmccd.PeakSearch(os.path.join(dirname, imagefilename),
                                                            PixelNearRadius=5,
                                                            removeedge=2,
    ...
    prefix, extens = dat_filename.split(".")
    # writing .cor file
    ...
    newUBmat_2, newUmat_2, newBmat_2, deviatoricstrain_2, RR_2, linkResidues_fit_2, pixdev_2, nb_fitted_peaks_2 = GetStrainOrient(
        newUBmat,
        Bmatrix,
        elem_label,
        emax_simul,
...
def GetCalibParameter(emax, veryclose_angletol, element, startingmatrix, defaultParam, data_exp,
    ...
    Twicetheta_theo, Chi_theo, Miller_ind_theo, posx_theo, posy_theo = data_theo
...
def Test_Sequential_fit():
    """
    sequential steps each comprising spots association and refinement

	TODO: branching is not clear and migration from main to function may have
	omit some variables initialisation

    """
    # define a scenario with small emax and tolerance and params to fit

    # dictionary of typical varying parameters combination
    dict_combinationparams = {}
    dict_combinationparams["all"] = "1" * 8
    dict_combinationparams["onlyrot"] = "0" * 5 + "1" * 3
    dict_combinationparams["onlydet"] = "1" * 5 + "0" * 3
    dict_combinationparams["Rot+2det"] = "01100111"  # xcen ycen +3 rot
    dict_combinationparams["Rot+4det"] = "01111111"  # xcen ycen xbet, xgam +3 rot
    dict_combinationparams["Rot+3det"] = "11100111"  # dd,xcen ycen +3 rot
    dict_combinationparams["xrot12"] = "01000110"  # xcen +2 rot
    dict_combinationparams["xrot13"] = "01000101"  # xcen +2 rot
    dict_combinationparams["yrot12"] = "00100110"  # ycen +2 rot
    dict_combinationparams["yrot13"] = "00100101"  # ycen +2 rot

    # scenario is a sequence of [emax, tolerance_angle, params_combination]
    scenario = []
    scenario.append([15, 1, "all"])
    scenario.append([25, 0.5, "all"])
    scenario.append([25, 0.5, "Rot+3det"])
    scenario.append([25, 0.5, "01100111"])
    scenario.append([25, 0.5, "all"])

    def elementary_step(emax, veryclose_angletol, startingmatrix, defaultParam, combi):
        ...
        return newparam[:5], newmat, pixDev, nb_of_pairs

    Params = [defaultParam[:5]]
    MAT = [startingmatrix]
    PixDev = [0]
    NB_pairs = [0]
    for scene in scenario:
        _emax, _veryclose_angletol, _combi = scene

        if _combi[0] not in ("0", "1"):  # need decoding
            _combi = dict_combinationparams[_combi]

        print("fitting step with emax= %.2f, ToleranceForPairs = %.3f, FittingParamsCombination = %s"
            % (_emax, _veryclose_angletol, _combi))

        newparamdet, newmat, pixDev, nb_of_pairs = elementary_step(_emax, _veryclose_angletol,
                                                                    MAT[-1], Params[-1], _combi)
        Params.append(newparamdet)
        MAT.append(newmat)
        PixDev.append(pixDev)
        NB_pairs.append(nb_of_pairs)
...
def test_SpotLinks():
    ...
    if res:
        refine_indexed_spots, linkedspots_link, linkExpMiller_link, linkIntensity_link, linkResidues_link, Energy_Exp_spot, fields = (
            res
        )
...
if __name__ == "__main__":
    ...
    newUBmat, newUmat, newBmat, deviatoricstrain, RR, linkResidues_fit, pixdev, nb_fitted_peaks, linkExpMiller_link = (
        res_strain
    )
    ...
    # in calcdef_include
    epsp = matstarlab_to_epsp_sample(
        matstarlab, np.zeros(5), omega0=40.0
    )  # calib1 = zros(5)
    print("Odile eps in sample frame", epsp)
    ...
    newUBmat, newUmat, newBmat, deviatoricstrain, RR, linkResidues_fit, pixdev, nb_fitted_peaks = (
        res_strain
    )
